...
def test_disabled_proxy_is_direct(monkeypatch):
    ...
    # Avoid real network: stub the ip check.
    monkeypatch.setattr("app.portal.proxy.check_exit_ip", lambda s, close=False: ("1.2.3.4", "IN"))
    acq = mgr.acquire()
...
def test_cooldown_rerolls_then_succeeds(monkeypatch):
    ...
    monkeypatch.setattr("app.portal.proxy.check_exit_ip",
                        lambda s, close=False: (next(ips, "10.0.0.2"), "IN"))
...
def test_fallback_when_not_sticky(monkeypatch):
    cfg = ProxyConfig(enabled=True, user="u", password="p", require_country="IN")
    monkeypatch.setattr("app.portal.proxy.check_exit_ip", lambda s, close=False: ("9.9.9.9", "IN"))
